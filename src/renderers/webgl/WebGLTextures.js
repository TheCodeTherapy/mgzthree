...
function WebGLTextures( _gl, extensions, state, properties, capabilities, utils, info ) {
	...
	function allocateTextureUnit() {
		...
		if ( textureUnit >= maxTextures ) {
			...
			console.warn( 'WebGLTextures: Trying to use ' + textureUnit + ' texture units while this GPU supports only ' + maxTextures );
	...
	function setTextureParameters( textureType, texture, supportsMips ) {
		...
		if ( supportsMips ) {
			...
			if ( texture.wrapS !== ClampToEdgeWrapping || texture.wrapT !== ClampToEdgeWrapping ) {
				...
				console.warn( 'WebGLRenderer: Texture is not power of two. Texture.wrapS and Texture.wrapT should be set to THREE.ClampToEdgeWrapping.' );
			...
			if ( texture.minFilter !== NearestFilter && texture.minFilter !== LinearFilter ) {
				...
				console.warn( 'WebGLRenderer: Texture is not power of two. Texture.minFilter should be set to THREE.NearestFilter or THREE.LinearFilter.' );
	...
	function setupRenderTarget( renderTarget ) {
		...
		if ( isWebGL2 && renderTarget.texture.format === RGBFormat && ( renderTarget.texture.type === FloatType || renderTarget.texture.type === HalfFloatType ) ) {
			...
			console.warn( 'WebGLRenderer: Rendering to textures with RGB format is not supported. Using RGBA format instead.' );
